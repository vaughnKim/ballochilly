...
// 		reply board
		function replyBoard() {
			$("#reply_board_Btn").click(function() {
				...
				location.href = "./videoAddBoard.do?pId=${boardlist.BV_SEQ_NO}&lvl=${boardlist.LVL}";
			});
		}
...
// 		edit board
		function editBoard() {
			...
			$("#complete_edit_board_Btn").click(function(){
				emptyEditBoard();
				$("form").ajaxSubmit({
					url : "./editVideoBoard.do",
					type : "post",
					success : function(result){
						alert(result.msg)
						location.href="./videoDetailBoard.do?bvSeqNo=${boardlist.BV_SEQ_NO}";
					}
				});
				return false;
			});
...
// 		delete board
		function deleteBoard() {
			$("#delete_board_Btn").click(function() {
				...
				if(alarm) {
					var bvSeqNo = $("input[name=bvSeqNo]").val();
// 					alert(bvSeqNo);
					$.ajax({
						url : "./deleteVideoBoard.do",
						type : "post",
						data : {"bvSeqNo" : bvSeqNo},
						success : function(result){
							alert(result.msg);
							location.assign("./videoBoard.do");
						}
					});
				}
			});	
...
// 				alert(lvl);
				$.ajax({
					...
					success : function(result){
						alert(result.msg)
						location.assign("./videoDetailBoard.do?bvSeqNo=${boardlist.BV_SEQ_NO}");
					}
				});
			});
		}
		...
		function addCmtOfCmt() {
			...
			$(".add_cmtOfcmt_Btn").click(function(){
				...
				$.ajax({
					url : "./addCommentVideoBoard.do",
					type : "post",
					data : {"id" : id, "bvSeqNo" : bvSeqNo, "content" : content, "pId" : pId, "lvl" : lvl},
					success : function(result){
						alert(result.msg)
						location.assign("./videoDetailBoard.do?bvSeqNo=${boardlist.BV_SEQ_NO}");
					}
...
// 		delete comment
		...
		function paging() {
			$("#goStartPage").click(function(){
				...
				var bvSeqNo = "${boardlist.BV_SEQ_NO}";
				location.href = "videoDetailBoard.do?page=" +page+"&bvSeqNo="+bvSeqNo;
			});
    ...
    <div class="row">
				...
				<table class="table table-striped table-bordered table-hover text-muted no-text-center" style="width:70%;">	
					<tr>
						<td id="show_file" width="399" colspan="2" height="200" align="left" >
							<c:if test="${video[0].BV_ATCH_VNAME != null}">
								<c:forEach var="video" items="${video}">
									
									<video src="/video/${video.BV_ATCH_VNAME}" width="600px" height="550px" controls></video> 
								</c:forEach>
							</c:if>
							<BR/>
							<strong>
      ...
      <!-- 	edit content		 -->
			            <td width="399" colspan="2" height="200" class="edit_board_content_area hidething">
							...
							<BR/>
			            	<textarea name="content" cols="110" rows="13" id="edit_board_content" maxlength="1000">${boardlist.CONTENT}</textarea>
			            	<br/>
			            </td>
					</tr>
		            <tr id="countComment">
						...
						<td style="position:relative;">
						  ...
						  <c:if test="${comment.ID == user.ID}">					
							<span style="position:absolute; right:60px;">
								<input type="button" value="수정" name="edit_comment_Btn" id="S_Btn" class="edit_comment_Btn" idx="${status.count}" />
							</span>
							<span style="position:absolute; right:10px;">
								<input type="button" value="삭제" name="delete_comment_Btn" id="S_Btn" class="delete_comment_Btn" idx="${status.count}"/>
							</span>
							<span style="position:absolute; right:80px;">
								<input type="button" value="수정완료" name="complete_comment_Btn" id="S_Btn" class="complete_comment_Btn hidething" idx="${status.count}" />
							</span>
							<span style="position:absolute; right:10px;">
								<input type="button" value="수정취소" id="S_Btn" class="cancel_comment_Btn hidething" idx="${status.count}" onclick="location.reload()" />
							</span>
							...
							<label class="show_comment_lower for_edit_comment" idx="${status.count}">
								<span style="font-weight:400; font-size:14px">${comment.CONTENT}
							</label>
				...
				<c:if test="${user.ID != null}">	
					<tr class="add_new_comment">
						<td align="center">
							...
							<textarea name="reply" id="add_new_comment" cols="120" rows="6" maxlength="85"></textarea>
...
<!-- 	all buttons  -->
			...
			<c:if test="${user.ID != null}">	
				<input type="button" value="글쓰기" id="write_board_Btn" class="L_Btn" onclick="javascript:location.href='./videoAddBoard.do'"/>
			...
			<c:if test="${user.ID != null && boardlist.ID == user.ID}">	
				...
				<input type="button" value="수정취소" id="cancel_edit_board_Btn" class="L_Btn hidething" onclick="javascript:location.href='./videoDetailBoard.do?bvSeqNo=${boardlist.BV_SEQ_NO}'"/>
				<input type="button" value="목록" onclick="javascript:location.href='./videoBoard.do'" class="L_Btn"/>
				<input type="hidden" value="${boardlist.BV_SEQ_NO}" name="bvSeqNo" />
			</div>							
			</form>
		</div>
	</div>	
